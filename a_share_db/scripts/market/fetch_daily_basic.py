#!/usr/bin/env python3
"""Fetch A-share daily basic indicators from Tushare.
...
from a_share_db.constant.daily_basic import (
    DAILY_BASIC_WINDOW_CALENDAR_DAYS,
    DAILY_BASIC_COLUMNS,
    TUSHARE_DAILY_BASIC_FIELDS,
    TUSHARE_MARKET_VALUE_TO_LOCAL,
    TUSHARE_SHARE_TO_LOCAL,
)
from a_share_db.constant.paths import (
    BACKUP_ROOT,
    DAILY_BASIC_ROOT,
    ETL_LOG_PATH,
    RAW_TUSHARE_DAILY_BASIC_ROOT,
    STOCK_BASIC_PATH,
...
DEFAULT_RAW_OUTPUT_ROOT = RAW_TUSHARE_DAILY_BASIC_ROOT
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch Tushare daily_basic data and build "
            "data/market_data/daily_basic/{code}.csv."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--with-raw",
        action="store_true",
        help="Also write raw Tushare daily_basic CSV files.",
    )
    parser.add_argument(
        "--limit-stocks",
        type=int,
...
def parse_tushare_query_date(value: str) -> datetime:
    try:
        ...
        raise ValueError(f"Date must use YYYYMMDD format: {value}") from exc


def iter_date_windows(start_date: str | None, end_date: str | None):
    if not start_date or not end_date:
        yield start_date, end_date
        return

    start = parse_tushare_query_date(start_date)
    end = parse_tushare_query_date(end_date)
    if start > end:
        raise ValueError("start-date must be earlier than or equal to end-date.")

    current = start
    while current <= end:
        window_end = min(current + timedelta(days=DAILY_BASIC_WINDOW_CALENDAR_DAYS - 1), end)
        yield current.strftime("%Y%m%d"), window_end.strftime("%Y%m%d")
        current = window_end + timedelta(days=1)


def fetch_single_window(token: str, ts_code: str, start_date: str | None, end_date: str | None):
    ...
    frame = pro.daily_basic(
        ...
        fields=",".join(TUSHARE_DAILY_BASIC_FIELDS),
    )
    pd = import_pandas()
    if frame is None:
        frame = pd.DataFrame(columns=TUSHARE_DAILY_BASIC_FIELDS)
    for column in TUSHARE_DAILY_BASIC_FIELDS:
        if column not in frame.columns:
            frame[column] = ""
    return frame[TUSHARE_DAILY_BASIC_FIELDS]


def fetch_from_tushare(token: str, ts_code: str, start_date: str | None, end_date: str | None):
    frames = [
        fetch_single_window(token, ts_code, window_start, window_end)
        for window_start, window_end in iter_date_windows(start_date, end_date)
    ]
    pd = import_pandas()
    if not frames:
        return pd.DataFrame(columns=TUSHARE_DAILY_BASIC_FIELDS)
...
def convert_tushare_daily_basic(raw):
    """Convert raw Tushare daily_basic rows into the local daily_basic schema."""
    pd = import_pandas()
    ...
    output["turnover_rate"] = raw["turnover_rate"]
    output["turnover_rate_free_float"] = raw["turnover_rate_f"]
    output["volume_ratio"] = raw["volume_ratio"]
    output["pe"] = raw["pe"]
    output["pe_ttm"] = raw["pe_ttm"]
    output["pb"] = raw["pb"]
    output["ps"] = raw["ps"]
    output["ps_ttm"] = raw["ps_ttm"]
    output["dividend_yield"] = raw["dv_ratio"]
    output["dividend_yield_ttm"] = raw["dv_ttm"]
    output["total_shares"] = pd.to_numeric(raw["total_share"], errors="coerce") * TUSHARE_SHARE_TO_LOCAL
    output["float_shares"] = pd.to_numeric(raw["float_share"], errors="coerce") * TUSHARE_SHARE_TO_LOCAL
    output["free_float_shares"] = pd.to_numeric(raw["free_share"], errors="coerce") * TUSHARE_SHARE_TO_LOCAL
    output["total_market_value"] = (
        pd.to_numeric(raw["total_mv"], errors="coerce") * TUSHARE_MARKET_VALUE_TO_LOCAL
    )
    output["float_market_value"] = (
        pd.to_numeric(raw["circ_mv"], errors="coerce") * TUSHARE_MARKET_VALUE_TO_LOCAL
    )
    output["update_time"] = update_time

    output = output[DAILY_BASIC_COLUMNS]
...
def run_daily_basic_etl(
    ...
    try:
        ...
        progress = ProgressReporter(stock_count, every=progress_every, label="Fetch daily_basic")
        ...
        for index, stock in enumerate(stocks.to_dict("records"), start=1):
            ...
            normalized = convert_tushare_daily_basic(raw)
...
def main() -> int:
    ...
    try:
        ...
        print(f"fetch_daily_basic failed: {exc}", file=sys.stderr)
        return 1
